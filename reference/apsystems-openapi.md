...
| **2005** | **MONTHLY quota reached — ~1000 API calls per month per app account** (documented by emlynmac/apsystems-openapi README: "APsystems enforces a limit of 1000 API calls per month"; codes 2005/7001/7002/7003 all map to "monthly quota or server rate limit"). No official reset hour; "data resumes when the limit resets" → assume calendar month. Our history: the old hourly cron (~730/mo) ran under it for months; the 2026-09-04 backfill spent the remaining balance overnight → locked from 2026-09-05 ~01:00 EDT; still locked Sept 6 (daily-reset and rolling-24 h theories falsified). **Expected unlock: 2026-10-01.** | live data is immune (local Modbus primary); daemon cloud extras time-gated to ~150/month (yesterday 2/day, summary ~1/day + month rollover, per-panel 2 windows/day); backfill cron capped at 300 calls/day — after unlock it finishes the archive in ~3 days within the same monthly envelope (3×300 + 150 ≈ 1050… on overage-risk months the cap self-limits via 2005 anyway) |
